...
private User user;
  ...
  public ResponseEntity<String> loginUser(@RequestParam("emailAddress") String emailAddress, @RequestParam("password") String password) {
    ResponseEntity<?> responseEntity = userService.loginUser(emailAddress, password);
...
String email=emailAddress;
      // Return the user ID along with the successful login response
      return ResponseEntity.ok(email);
    } else if (responseEntity.getStatusCode() == HttpStatus.UNAUTHORIZED) {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build(); // Unauthorized access
    } else {
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build(); // Other server error
    }
  }
  ...
  public ResponseEntity<?> getUserByEmailId(@RequestParam("email") String email) {
    ...
    ResponseEntity<?> user = userService.getUserByEmail(email);
