...
public class DoctorService implements BaseService {
  ...
  @Override
  public ResponseEntity<?> login(String emailAddress, String password) {
    // Delegate the login functionality to the repository service
    return doctorRepositoryService.login(emailAddress, password);
  }
  public ResponseEntity<String>  getDoctorByEmail(String email) {
    // Implement the logic to fetch the user by email from the database
    return (ResponseEntity<String>) doctorRepositoryService.findByEmailAddress(email);
  }
}
