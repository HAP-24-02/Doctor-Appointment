...
public class UserRepositoryServiceImpl implements UserRepositoryService {
  ...
  public ResponseEntity<String> loginUser(@RequestParam("emailAddress") String emailAddress, @RequestParam("password") String password) {
    ...
    if (user != null && user.getPassword().equals(password)) {
     ...
     return (ResponseEntity<String>) ResponseEntity.ok(id);
    // Password is incorrect or user does not exist
// Password is correct
    } else {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build(); // Password is incorrect or user does not exist
    }
  }
  @Override
  public ResponseEntity<?> findByEmailAddress(String emailAddress) {
    User user = userRepository.findByEmailAddress(emailAddress);
